...
import {
  ...
  sessionNoteComposingValueSchema,
...
export interface IApiClient {
  sessions: {
    ...
    noteComposing(payload: RequestPayload<'session.noteComposing'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.noteComposing'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.noteComposing': sessionNoteComposingValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    noteComposing: (payload, signal) => this.callUnary('session.noteComposing', payload, signal),
